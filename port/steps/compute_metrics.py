...
def _compute_internal_coordinate_rmsd(
    ...
    qm_conformer = qm_conformer.m_as(unit.angstrom)
    mm_conformer = mm_conformer.m_as(unit.angstrom)

    geo_molecule = GeometricMolecule()
    geo_molecule.Data = {
        "resname": ["UNK"] * molecule.n_atoms,
        "resid": [0] * molecule.n_atoms,
        "elem": [SYMBOLS[atom.atomic_number] for atom in molecule.atoms],
    ...
    for ic_type, ic_values in internal_coordinates.items():
        ...
        if ic_type in ["Angle", "Dihedral", "Improper"]:
            rmsd = compute_rmsd(
                qm_values * 180 / numpy.pi, mm_values * 180 / numpy.pi, 360
            )
        else:
...
def _compute_metrics(
    label: str, options: Dict[str, Any], input_path, index: int
) -> pandas.DataFrame:
    record_id_tag = options["record-id-tag"]

    qm_tag = options["qm-tag"]
    mm_tag = options["mm-tag"]

    qm_molecule_stream = oechem.oemolistream(
        os.path.join(
            input_path,
            label.replace("OpenFF ", "openff_unconstrained-") + f"-{index+1}.sdf",
        )
    )
    qm_molecule_stream.SetConfTest(oechem.OEAbsoluteConfTest(False))

    mm_molecule_stream = oechem.oemolistream(
        options["mm-structures"][label] + f"-{index}.sdf"
    )
...
def compute_metrics(options, input_path, output_path, n_chunks: int):
    ...
    method_labels = [*options["mm-structures"]]
    ...
    for index in range(n_chunks):
        ...
        metrics.to_csv(f"{output_path}/03-metrics-{index}.csv", index=False)
